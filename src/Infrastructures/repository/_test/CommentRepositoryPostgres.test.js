...
describe('CommentRepositoryPostgres', () => {
  ...
  describe('checkCommentAvailability function', () => {
    ...
    it('should not throw NotFoundError when comment available', async () => {
      ...
      // Action
      const comment = await commentRepositoryPostgres.checkCommentAvailability(commentId);
      ...
      // expect(comment).toHaveProperty('thread', threadId);
      // expect(comment).toHaveProperty('owner', userId);
      // expect(comment).toHaveProperty('content');
      // expect(comment).toHaveProperty('deleted_at');
  ...
  describe('getCommentsByThreadId function', () => {
    it('should return thread comments correctly', async () => {
      ...
      expect(comments[0].id).toBe('comment-old');
      ...
      expect(comments[0].deleted_at).toBe(null);
      expect(comments[1].deleted_at).toBe(null);
    });
  });
  ...
  describe('deleteCommentById function', () => {
    it('should soft delete comment and update deleted_at field', async () => {
      ...
      // Additional assertion for deleted comment details
      expect(comments[0]).toHaveProperty('id', commentId);
      expect(comments[0]).toHaveProperty('thread', 'thread-123');
      expect(comments[0]).toHaveProperty('owner', 'user-123');
      expect(comments[0]).toHaveProperty('content', 'A new comment');
      expect(comments[0]).toHaveProperty('deleted_at');
    });
  });
});
