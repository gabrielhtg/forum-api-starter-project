...
describe('ReplyRepositoryPostgres', () => {
  ...
  describe('deleteReplyById function', () => {
    it('should soft delete reply and update deleted_at field', async () => {
      ...
      expect(replies).toHaveLength(1); // Ensure the reply still exists
      expect(replies[0].deleted_at).toBeTruthy(); // Ensure deleted_at field is set

      // Ensure deleted_at contains a valid date
      expect(new Date(replies[0].deleted_at).getTime()).not.toBeNaN(); // Check if it's a valid date

      // Additional assertion for reply details
      expect(replies[0]).toHaveProperty('id', replyId); // Check if ID matches
      expect(replies[0]).toHaveProperty('comment', 'comment-123'); // Ensure comment ID is correct
      expect(replies[0]).toHaveProperty('owner', 'user-123'); // Ensure owner is correct
    });
  });
});
